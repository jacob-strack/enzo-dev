...
int grid::AddInitialMagneticSupernovaeToList()
{
  ...
  for (int star_id = 0;  star_id < this->NumberOfParticles; star_id++){
    if (this->ParticleType[star_id] == PARTICLE_TYPE_STAR){
      ...
      star_lifetime = this->ParticleAttribute[1][star_id];
      sn_birthtime = star_birthtime + star_lifetime;
      if(star_birthtime > 0) //only place seed fields from initial star particles 
	      continue; 
      //find the position of the particle. check if it is in the disk of the galaxy 
      int x = (this->ParticlePosition[0][star_id] / CellWidth[0][0]) + NumberOfGhostZones; 
      int y = (this->ParticlePosition[1][star_id] / CellWidth[1][0]) + NumberOfGhostZones; 
      int z = (this->ParticlePosition[2][star_id] / CellWidth[2][0]) + NumberOfGhostZones; 
      int ind = (z * this->GridDimension[1] + y) * this->GridDimension[0] + x;
      int isDiskNum = FindField(isDisk, FieldType, NumberOfBaryonFields); 
      if(!BaryonField[isDiskNum][ind])
	     continue;  
      // Creates a supernova with magnetic feedback set by user-defined parameters and                              
      // adds it to supernova list                                                                                  
      if((this->Time > sn_birthtime) && (this->Time < sn_birthtime + sn_duration)){
